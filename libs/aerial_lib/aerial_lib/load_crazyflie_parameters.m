...
param.Q = diag([40, 40, 40, 0, 0, 0, 0, 0, 40]);
param.R = diag([160, 4, 4, 2]);
%param.Q = diag([40, 40, 1, 0, 0, 0, 0, 0, 10]);
%param.R = diag([4, 4, 4, 1]);

[param.K_lqr_outer_loop, param.P] = dlqr(Ad, Bd, param.Q, param.R);
...
param.Adist = [Ad dist_vec; zeros(param.d, param.n + param.d)];
Bdist = [Bd; zeros(param.d, param.m)];
param.Cdist = [Cd zeros(param.p, param.d)];
Ddist = Dd;

%% INPUT/OUTPUT BOUNDS %%
%% Input constraints
thrust_min = 0.25 * sum(param.nrotor_vehicle_thrust_max);
